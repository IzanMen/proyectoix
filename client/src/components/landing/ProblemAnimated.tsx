...
function BadBrowserContent() {
  const loadingCtrl = useAnimation();
  const contentCtrl = useAnimation();
  const meVoyCtrl = useAnimation();
  const seVaCtrl = useAnimation();
  ...
  useEffect(() => {
    ...
    const run = async () => {
      while (alive.current) {
        loadingCtrl.set({ opacity: 1 });
        contentCtrl.set({ opacity: 0 });
        meVoyCtrl.set({ opacity: 0, scale: 0.85 });
        seVaCtrl.set({ opacity: 0, x: 0 });

        if (!alive.current) return;
        loadingCtrl.start({ opacity: 0, transition: { delay: 1.3, duration: 0.5 } });
        await contentCtrl.start({ opacity: 1, transition: { delay: 1.5, duration: 0.4 } });

        if (!alive.current) return;
        meVoyCtrl.start({
          opacity: [0, 1, 1, 0],
          scale: [0.85, 1, 1, 0.85],
          transition: { delay: 0.3, duration: 2.6, times: [0, 0.15, 0.7, 1] },
        });
        await delay(3200);

        if (!alive.current) return;
        await seVaCtrl.start({
          opacity: [0, 1, 1, 0],
          x: [0, 30, 60, 200],
  ...
  }, [loadingCtrl, contentCtrl, meVoyCtrl, seVaCtrl]);
...
function GoodBrowserContent() {
  const navCtrl = useAnimation();
  const line1Ctrl = useAnimation();
  const line2Ctrl = useAnimation();
  const line3Ctrl = useAnimation();
  const line4Ctrl = useAnimation();
  const btnCtrl = useAnimation();
  const pingCtrl = useAnimation();
  const valeCtrl = useAnimation();
  const alive = useRef(true);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      while (alive.current) {
        navCtrl.set({ opacity: 0, y: 8 });
        line1Ctrl.set({ opacity: 0, y: 6 });
        line2Ctrl.set({ opacity: 0, y: 6 });
        line3Ctrl.set({ opacity: 0, y: 6 });
        line4Ctrl.set({ opacity: 0, y: 6 });
        btnCtrl.set({ opacity: 0, y: 10, scale: 0.95 });
        pingCtrl.set({ opacity: 0, scale: 0 });
        valeCtrl.set({ opacity: 0, scale: 0.85 });

        if (!alive.current) return;
        await navCtrl.start({ opacity: 1, y: 0, transition: { duration: 0.4 } });

        if (!alive.current) return;
        line1Ctrl.start({ opacity: 1, y: 0, transition: { delay: 0.1, duration: 0.35 } });
        await delay(150);
        line2Ctrl.start({ opacity: 1, y: 0, transition: { duration: 0.35 } });
        await delay(150);
        line3Ctrl.start({ opacity: 1, y: 0, transition: { duration: 0.35 } });
        await delay(150);
        await line4Ctrl.start({ opacity: 1, y: 0, transition: { duration: 0.35 } });
        ...
        pingCtrl.start({
          opacity: [0, 1, 1, 1],
          scale: [0, 1.4, 1, 1],
          transition: { delay: 0.2, duration: 0.6, times: [0, 0.5, 0.8, 1] },
        });
        await delay(1200);
  ...
  }, [navCtrl, line1Ctrl, line2Ctrl, line3Ctrl, line4Ctrl, btnCtrl, pingCtrl, valeCtrl]);
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 px-3 py-2 bg-[#0f0f0f] border-b border-white/5">
        ...
        <span className="w-2.5 h-2.5 rounded-full bg-white/20" />
        <span className="w-2.5 h-2.5 rounded-full bg-white/20" />
        <span className="ml-3 text-[10px] font-mono text-white/30 truncate">web-cualquiera.com</span>
      </div>
      ...
      <div className="px-4 py-3 bg-[#0f0f0f] border-t border-red-500/10 flex items-center justify-between text-[11px]">
        <span className="text-red-300/80 font-mono uppercase tracking-widest">Web antigua</span>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 px-3 py-2 bg-[#0f0f0f] border-b border-white/5">
        ...
        <span className="ml-3 text-[10px] font-mono text-white/40 truncate">tu-negocio.com</span>
      </div>
      ...
      <div className="px-4 py-3 bg-[#0f0f0f] border-t border-[hsl(270,100%,60%)]/15 flex items-center justify-between text-[11px]">
        <span className="text-[hsl(270,100%,75%)] font-mono uppercase tracking-widest">Tu nueva web</span>
